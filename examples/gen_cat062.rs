//! Encode a CAT-062 message with libasterix and write it to `tests/cat062.bin`.
//!
//! The companion test `tests/test_cat062.py` decodes this file with an
...
fn main() {
    ...
    std::fs::write("tests/cat062.bin", &bytes).expect("write tests/cat062.bin");
    println!("wrote tests/cat062.bin ({} bytes)", bytes.len());
}
